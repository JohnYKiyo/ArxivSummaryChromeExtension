...
ツールバーの拡張機能アイコンをクリックしてポップアップを開き、下部の「設定」パネルで以下を入力して「保存」します (API Key 未設定の間は設定パネルが自動で開きます):

- **API Endpoint**: ローカル開発では `http://localhost:8000`
- **Google API Key**: 拡張機能はリクエストごとに `X-Google-Api-Key` ヘッダで送信します。未入力の間は「翻訳を開始」ボタンが有効になりません

### 動作確認

> **Note**: 拡張機能はポップアップ専用です (content script は無いため、arXiv のページ上にボタンは表示されません)。

1. [arXiv](https://arxiv.org/) の論文ページ (例: `https://arxiv.org/abs/2301.00001`) を開く
2. ツールバーの拡張機能アイコンをクリックしてポップアップを開く — アクティブタブが arXiv のページなら URL が自動入力されます (それ以外のタブから開いた場合は手動で貼り付け)
3. 「翻訳を開始」をクリック
4. 進捗バーで完了を確認し、「ダウンロード (ZIP)」をクリック
